...
class DungeonState:
    ...
    def __init__(self, manager):
        ...
        # tracks where a held pinch/mouse button currently is the shield
        # can lock its color/size while that same press is still down (see
        # Shield.note_still_pressed), None the instant it's released
    ...
    def _start_combat(self):
        self.phase = "combat"
        self.gate = None
        # one enemy per color for gameplay showcase
    ...
    def on_update(self, dt):
        ...
        if self._pointer_down_at is not None:
            self.shield.note_still_pressed(*self._pointer_down_at)
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if self.gun_hud.on_mouse_press(x, y, button, modifiers):
            return
        if self.gem is not None and self.gem.on_mouse_press(x, y):
            return
